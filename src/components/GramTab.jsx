  ...
  return wrap(
    <>
      ...
      <div style={{ background: C.black, borderRadius: 14, padding: "18px 16px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 14 }}>
          ...
          <div style={{ textAlign: "right" }}>
            ...
            {/* Streak pod punktami */}
            <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,.5)", marginTop: 4 }}>
              🔥 {gameData.streak_current || 0} dni z rzędu
            </div>
          </div>
        </div>
      ...
      <div style={{ background: C.white, borderRadius: 12, padding: "16px 18px" }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: C.greyMid, letterSpacing: .5, marginBottom: 10, textTransform: "uppercase" }}>
          Jak zdobywać punkty?
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {[
            { text: "Tip dnia",          sub: "10 pkt · 6 dni w tygodniu" },
            { text: "Quiz tygodniowy",   sub: "max 60 pkt + premia za szybkość" },
            { text: "Kolejne poziomy",   sub: "odblokuj odznaki za punkty" },
            { text: "Nagrody",           sub: "wymieniaj punkty na szkolenia" },
          ].map(({ text, sub }) => (
            <div key={text} style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 4, alignSelf: "stretch", background: C.green, borderRadius: 2, flexShrink: 0 }}/>
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: C.black, lineHeight: 1.2 }}>{text}</div>
                <div style={{ fontSize: 11, color: C.greyMid, marginTop: 1 }}>{sub}</div>
              </div>
            </div>
          ))}
        </div>
        <div style={{ marginTop: 14, paddingTop: 12, borderTop: `1px solid ${C.grey}`, textAlign: "center", fontSize: 12, fontWeight: 700, color: C.green, letterSpacing: .3 }}>
          Odwiedzaj nas codziennie i nie przerywaj serii!
        </div>
      </div>
    </>
  );
