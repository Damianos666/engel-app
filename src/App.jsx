...
function AppRoot() {
  ...
  useEffect(() => {
    ...
    // NAPRAWA BUGU: Było 24 _60_ 60_000 (24 GODZINY!) — wiadomości sprawdzane raz na dobę.
  ...
  const setTrainerView = useCallback(
    async (v) => {
      setTrainerViewRaw(v);
      setTab(0);
      try {
        ...
        logErr("[TRAINER VIEW] save error:", e.message);
      }
    },
    [user]
  );
  ...
  const handleLogin = useCallback(async (rawUser) => {
    ...
    try {
      ...
      if (Array.isArray(u.active_groups) && u.active_groups.length) setActiveGroups(u.active_groups);
      ...
      overrides.forEach((ov) => {
        overridesMap[ov.training_id] = ov;
      });
  ...
  const handleComplete = useCallback(
    async (entry) => {
      ...
      try {
        ...
        const updated = await db.update(user.accessToken, "completions", `user_id=eq.${user.id}&training_id=eq.${entry.training.id}`, payload);
  ...
  const progress = useMemo(() => calcProgress(completed, activeGroups), [completed, activeGroups]);

  const bannerSub = useMemo(() => [user?.displayRole, user?.firma].filter(Boolean).join(" · "), [user?.displayRole, user?.firma]);

  // BEZPIECZEŃSTWO + OPTYMALIZACJA: Wartość kontekstu memoizowana
  ...
  return (
    <div className="app-container" style={styles.appContainer}>
      ...
      <div style={styles.banner}>
        <span style={styles.bannerName}>{user.displayName}{bannerSub ? `· ${bannerSub}` : ""}</span>
    ...
    <div style={styles.tabBar}>
      {TRAINER_TABS.map(([label, icon], i) => (
        ...
        >
          ...
          <span style={{ fontSize: 10, fontWeight: 600, color: tab === i ? C.black : C.greyMid, letterSpacing: 0.2 }}>{label}</span>
        </button>
      ))}
    </div>
  );
